<script lang="ts">
	...
	import { OnFileDrop, OnFileDropOff, BrowserOpenURL } from '../../wailsjs/runtime/runtime';
	...
	onMount(() => {
		...
		if (savedMaxResults != null) {
			maxVisibleMatches = parseInt(String(savedMaxResults), 10);
		}
		const savedThreshold = getSetting('confidenceThreshold');
		if (savedThreshold != null) {
			confidenceThreshold = parseFloat(String(savedThreshold));
		}
		const savedMaxTotal = getSetting('maxTotalResults');
		if (savedMaxTotal != null) {
			maxTotalResults = parseInt(String(savedMaxTotal), 10);
		}
		...
		// Listen for files passed on app startup (e.g. dropped onto exe/shortcut)
		EventsOn('app:openFile', (p: string) => {
			...
			>
				...
				>
					<div class="flex-1 flex flex-col items-center justify-center">
						<FileUp
							class="h-16 w-16 mx-auto mb-4 opacity-50 group-hover:scale-105 transition-all group-hover:-translate-y-1"
						/>
				...
				<div class="max-w-4xl w-full mx-auto space-y-4">
					...
					<div class="flex items-center justify-between mt-1 text-wrap">
						...
						<button
							...
							aria-label={m['home.scan_another']()}>{m['home.scan_another']()}</button
						>
					</div>
										...
										<div class="mt-2 space-y-2">
											<div>
												<button
													...
													on:click={() =>
														copyToClipboard(scanResult?.bestMatch?.name || '', 'best-name')}
											...
											{#if scanResult.bestMatch.description}
												<button
													...
													on:click={() =>
														copyToClipboard(
															scanResult?.bestMatch?.description || '',
															'best-description'
														)}
													aria-label={m['home.copy']()}
												>
													<span class="hover:opacity-60 opacity-80 inline"
														>{scanResult.bestMatch.description}</span
													>
												</button>
											{/if}
												...
												{#if scanResult.bestMatch.mimeType}
													<div>
														...
														<button
															...
															on:click={() =>
																copyToClipboard(scanResult?.bestMatch?.mimeType || '', 'best-mime')}
															aria-label={m['home.copy']()}
														>
															<span class="hover:opacity-80 inline"
						...
						<div class="card bg-base-200">
							<div class="card-body">
								...
								<div class="space-y-2 mt-2">
									{#each scanResult.matches.slice(1, showAllMatches ? undefined : maxVisibleMatches + 1) as match, index (match.name + '-' + (match.extension || '') + '-' + index)}
										<div class="border-l-2 border-base-300 pl-3 py-1">
												...
												<div>
													...
													{#if match.extension}
														<button
															...
															data-tip={copiedItem === `match-${index}-ext`
																? m['home.copied']()
																: ''}
															on:click={() =>
																copyToClipboard(match.extension || '', `match-${index}-ext`)}
															aria-label={m['home.copy']()}
														>
															.{match.extension}
														</button>
											...
											{#if match.description}
												<button
													...
													on:click={() =>
														copyToClipboard(match?.description || '', 'match-description')}
													aria-label={m['home.copy']()}
												>
									...
									{#if scanResult.totalMatches > maxVisibleMatches + 1 && !showAllMatches}
										<button
											...
											aria-label={m['home.show_more_matches']({
												count: scanResult.totalMatches - maxVisibleMatches - 1
											})}
										>
											{m['home.more_possible_matches']({
												count: scanResult.totalMatches - maxVisibleMatches - 1
											})}.
										</button>
									{/if}
							...
							<div class="card-body">
								<div class="flex flex-col items-center text-center gap-4 py-8">
									...
									<button
										...
										aria-label={m['home.scan_another']()}
									>
										{m['home.scan_another']()}
									</button>
								</div>
