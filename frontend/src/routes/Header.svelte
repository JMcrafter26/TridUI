...
<div class="fab">
  <div tabindex="0" role="button" class="btn btn-soft btn-lg btn-circle btn-primary {$updateAvailable && $updateAvailable.updateAvailable ? 'indicator' : ''}">
  ...
  <div class="fab-close">
	...
	<a class="btn btn-lg btn-circle {$updateAvailable && $updateAvailable.updateAvailable ? 'indicator' : ''}" href="/about" title="{m["header.about"]()}" aria-label="{m["header.about"]()}">
		{#if $updateAvailable && $updateAvailable.updateAvailable}
			<span class="indicator-item badge badge-secondary badge-xs mt-1.5 mr-1.5"></span>
		{/if}
		<Info />
